...
class ResBlock(nn.Module):
    ...
    def __init__(self, input_channels, output_channels, use_1x1conv=False, strides=1):
        ...
        if use_1x1conv:
            ...
            self.conv3 = None
        self.bn1 = nn.BatchNorm2d(output_channels,eps=1e-04)
        self.bn2 = nn.BatchNorm2d(output_channels,eps=1e-04)
...
class MyResNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.bl1 = nn.Sequential(nn.Conv2d(3, 64, kernel_size=7,stride=2),
                                 nn.BatchNorm2d(64,eps=1e-04), nn.ReLU(),
                                 nn.MaxPool2d(kernel_size=3)
                                 )
        self.bl2 = nn.Sequential(*self.res_layer_maker(32, 64,2, first_block=True))
        ...
        # self.pool1 = nn.MaxPool2d((2, 2))#源代码为(1,1)
    ...
    def forward(self, x):
        ...
        x = self.bl4(x)
        x = self.bl5(x)
        x =  F.leaky_relu(self.linear(self.flatten(self.pool1(x))))
        return x
    ...
    def train_net(self, dataloader, epochs=2, device=None, criterion='CrossEntropyLoss',
        ...
        if optimizer == 'SGD':
            optimizer = optim.SGD(self.parameters(), lr=0.0005)
        elif optimizer == 'Adam':
        ...
        for epoch in range(epochs):  # 对数据集循环多次
            ...
            for i, data in enumerate(dataloader, 0):
                ...
                # 在Pytorch中，使用to(device)函数可以将张量（Tensor）或模型（Model）移动到指定的设备上，如CPU、GPU或TPU
    ...
    def test_net(self, testloader, net,classes, device=None):
        """
        在测试集上测试网络
        ...
        output = self(x)
        # 准备计算总正确率和各类正确率,初始化，每个类的概率一开始都赋值为0
        ...
        # 不需要计算梯度，节省资源
        testloaderbox=[]
        # 使用with torch.no_grad(): 表明当前计算不需要反向传播，因为是测试，直接使用模型参数即可
        with torch.no_grad():
            for data in testloader:
                ...
                for label, prediction in zip(labels, predictions):
                    if label == prediction:
                        correct += 1
                        # correct_pred为字典，包含n个键值对，correct_pred[classes[label]]取到对应类的正确个数
                        correct_pred[classes[label]] += 1
                    total += 1
                    total_pred[classes[label]] += 1
                testloaderbox.append(correct/testloader.batch_size)
                print('testloaderbox',testloaderbox)
                correct=0
        return testloaderbox
