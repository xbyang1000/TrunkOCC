...
class DROCCTrainer:
    ...
    def test(self, test_loader, metric):
        ...
        if metric == 'F1':
            ...
            print("火焰预测精度", prec[0], "误警率", 1 - prec[0])
    ...
    def save(self, path, metric):
        torch.save(self.model.state_dict(), os.path.join(path, 'model.pt'.format(metric)))

    def load(self, path, modelname):
        self.model.load_state_dict(torch.load(os.path.join(path, modelname)))
